...
async fn main() -> Result<(), async_nats::Error> {
    let nats_url = std::env::var("NATS_URL") //connection to nats
    ...
    // create a stream and configuring
    ...
    jetstream
        ...
        .await?;
    jetstream
        .publish("events.us.mouse_clicked", "".into())
        .await?
        .await?;
    jetstream
    ...
    // created consumer and configureing
    println!(
        "Stream info without any consumer: {:#?}",
        stream.info().await?
    );
    ...
    let err = stream
        ...
        .await
        .expect_err("fail to create ovrrlapping consumer");
